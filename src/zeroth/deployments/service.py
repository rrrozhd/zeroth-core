...
class DeploymentService:
    ...
    async def deploy(
        ...
        for _ in range(3):
            ...
            tenant_id = str(
                graph.deployment_settings.get("tenant_id", "default")
            )
        ...
        raise DeploymentError(
            f"failed to allocate deployment version for {deployment_ref!r} "
            "after retries"
        ) from last_error
    ...
    async def rollback(
        ...
        return await self.deploy(
            deployment_ref, current.graph_id, target_graph_version
        )

    async def _require_published_graph(
        self, graph_id: str, version: int | None
    ) -> Graph:
        ...
        if graph is None:
            if version is None:
                raise DeploymentError(
                    f"graph {graph_id} has no published versions to deploy"
                )
            raise KeyError(f"{graph_id}@{version}")
        if graph.status is not GraphStatus.PUBLISHED:
            msg = (
                f"graph version {graph.graph_id}@{graph.version} must be "
                "published before deploy"
            )
    ...
    async def _latest_published_graph(
        ...
        for graph in reversed(
            await self.graph_repository.list_versions(graph_id)
        ):
    ...
    async def _ensure_deployment_ref_lineage(
        self, deployment_ref: str, graph_id: str
    ) -> None:
    ...
    def _entry_node(self, graph: Graph):
        ...
        for node in graph.nodes:
            if node.node_id == entry_step:
                return node
        msg = (
            f"graph {graph.graph_id}@{graph.version} has unknown entry "
            f"step {entry_step}"
        )
        raise DeploymentError(msg)

    async def _resolve_contract_version(
        self, contract_ref: str | None
    ) -> int | None:
        ...
        if self.contract_registry is None:
            raise DeploymentError(
                f"contract registry is required to deploy {contract_ref!r}"
            )
        try:
            contract = await self.contract_registry.resolve(
                ContractReference(name=contract_ref)
            )
