...
def _make_run(run_repo: RunRepository) -> Run:
    ...
    return run_repo.transition(persisted.run_id, RunStatus.RUNNING)


def test_dead_letter_not_triggered_below_threshold(sqlite_db: SQLiteDatabase) -> None:
    ...
    for _ in range(2):
        dead_lettered = manager.handle_run_failure(run.run_id)
...
def test_dead_letter_triggered_at_threshold(sqlite_db: SQLiteDatabase) -> None:
    run_repo = RunRepository(sqlite_db)
    manager = DeadLetterManager(run_repository=run_repo, max_failure_count=3)
    ...
    result = None
    for _ in range(3):
        result = manager.handle_run_failure(run.run_id)
...
def test_dead_letter_metadata_captured(sqlite_db: SQLiteDatabase) -> None:
    run_repo = RunRepository(sqlite_db)
    manager = DeadLetterManager(run_repository=run_repo, max_failure_count=1)

    run = _make_run(run_repo)
    manager.handle_run_failure(run.run_id)
...
def test_list_dead_letter_runs_returns_dead_lettered_runs(sqlite_db: SQLiteDatabase) -> None:
    run_repo = RunRepository(sqlite_db)
    manager = DeadLetterManager(run_repository=run_repo, max_failure_count=1)

    run1 = _make_run(run_repo)
    run2 = _make_run(run_repo)  # This one won't be dead-lettered.

    manager.handle_run_failure(run1.run_id)

    dead_letters = run_repo.list_dead_letter_runs(DEPLOYMENT)
