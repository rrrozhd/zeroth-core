...
def test_worker_picks_up_pending_run(sqlite_db) -> None:
    ...
    final = service.run_repository.get(run.run_id)
    assert final is not None
    # The run should have been claimed (moved to RUNNING or beyond).
    assert final.status is not RunStatus.PENDING


def test_run_with_no_agent_runner_does_not_crash_worker(sqlite_db) -> None:
...
def test_lease_prevents_double_claim(sqlite_db) -> None:
    ...
    def _claim(worker_id: str):
        run_id = lease_manager.claim_pending(service.deployment.deployment_ref, worker_id)
        if run_id is not None:
            claimed_run_ids.append(run_id)

    t1 = threading.Thread(target=_claim, args=("worker-1",))
    t2 = threading.Thread(target=_claim, args=("worker-2",))
    t1.start()
    t2.start()
    t1.join()
    t2.join()
