...
async def test_metrics_returns_200_with_prometheus_content_type(sqlite_db) -> None:
...
async def test_metrics_contains_queue_depth_gauge(sqlite_db) -> None:
    service, _ = await deploy_service(
        sqlite_db,
...
async def test_metrics_counter_increments_are_visible(sqlite_db) -> None:
    service, _ = await deploy_service(
        sqlite_db,
...
async def test_metrics_requires_metrics_permission(sqlite_db) -> None:
    service, _ = await deploy_service(
        sqlite_db,
...
async def test_metrics_hides_service_from_foreign_tenant_admin(sqlite_db) -> None:
